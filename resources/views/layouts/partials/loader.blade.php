...
<div class="loader-wrap">
    ...
    <div class="loader-wrap-heading">
        <div class="load-text">
            @foreach(str_split($settings['site_name'] ?? 'Maylancer') as $letter)
                @if($letter !== ' ')
                <span>{{ $letter }}</span>
                @endif
            @endforeach
...
<div class="tp-offcanvas-area">
    <div class="tp-offcanvas">
            ...
            <div class="tp-offcanvas-logo">
                <a href="{{ url('/') }}">
                    @if(!empty($settings['logo_dark']))
                        <img data-width="150" src="/storage/{{ $settings['logo_dark'] }}" alt="{{ $settings['site_name'] ?? config('app.name') }}">
        ...
        <div class="tp-offcanvas-content d-none d-xl-block">
            <h3 class="tp-offcanvas-title">{{ __('front.offcanvas.hello_title') }}</h3>
            <p>{{ $settings['site_description'] ?? __('front.offcanvas.description') }}</p>
        </div>
        <div class="tp-offcanvas-menu d-xl-none">
            <nav>
                <ul>
                    <li><a href="{{ url('/') }}">{{ __('front.footer.home') }}</a></li>
                    <li><a href="{{ route('portfolio.index') }}">{{ __('front.nav.portfolio') }}</a></li>
                    <li><a href="{{ route('blog.index') }}">{{ __('front.nav.blog') }}</a></li>
                    @foreach($headerPages ?? [] as $headerPage)
                    <li><a href="{{ route('page.show', $headerPage->slug) }}">{{ $headerPage->title }}</a></li>
                    @endforeach
                    <li><a href="{{ route('contact.show') }}">{{ __('front.nav.contact_us') }}</a></li>
                </ul>
            </nav>
        </div>
        ...
        <div class="tp-offcanvas-social">
            <h3 class="tp-offcanvas-title sm">{{ __('front.offcanvas.follow_us') }}</h3>
            <ul>
                ...
                <li>
                    <a href="{{ $settings['social_dribbble'] }}">
